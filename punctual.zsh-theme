...
punctualUser () {
    ...
    if [[ $USER == 'root' ]]; then
	    ...
	    punctualDecorate $(echo '%n' | tr -d '[:blank:]') "${PUNCTUAL_ROOT_USER_COLOUR}";
    else
        punctualDecorate $(echo '%n' | tr -d '[:blank:]') "${PUNCTUAL_USER_COLOUR}";
    fi;
}

punctualHostname () {
    echo -n '@';
    punctualDecorate $(echo '%m' | tr -d '[:blank:]') "${PUNCTUAL_HOSTNAME_COLOUR}";
}
